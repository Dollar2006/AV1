...
import { Peca } from "../../core/entidades/Peca";
import { TipoPeca } from "../../core/enums/TipoPeca";
import { StatusPeca } from "../../core/enums/StatusPeca";
import fs from 'fs'
import path from 'path'

export class PecaMenu {
    static pecas: Peca[] = [];
    ...
    static async listarPecas(): Promise<void> {
        ...
        this.pecas.forEach((peca, index) => {
            console.log(`${index + 1}. ${peca.getNome} - ${peca.getTipo} - ${peca.getFornecedor} - ${peca.getStatus}`);
        });
    }
    ...
    static async buscarPorNome(): Promise<void> {
        ...
        if (peca) {
            ...
            console.log(`Nome: ${peca.getNome}`);
            console.log(`Tipo: ${peca.getTipo}`);
