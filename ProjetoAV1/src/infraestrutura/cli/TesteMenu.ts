...
import { Teste } from "../../core/entidades/Teste";
import { TipoTeste } from "../../core/enums/TipoTeste";
import { ResultadoTeste } from "../../core/enums/ResultadoTeste";
import fs from 'fs'
import path from 'path'

export class TesteMenu {
    static testes: Teste[] = [];
        ...
        const { indice } = await inquirer.prompt([
            {
                ...
                choices: this.testes.map((teste, index) => ({
                    name: `${teste.tipo} - ${teste.resultado}`,
                    value: index
                }))
            }
        ]);
